...
> bim.house's client-side IFC uploader is currently broken (its import map is missing `three/examples/jsm/utils/BufferGeometryUtils`), so the full IFC can't be loaded there. A flattened 110-box version of v1.2.0 is uploaded to [bim.house — token bim_6a00662b24f97ygo](https://bim.house/viewer/bim?edit=1&token=bim_6a00662b24f97ygo) (no curtain-wall semantics, no light fixtures, no IfcGrid — but the building, furniture, river and trees all render).
